...
print("\n Replaced by Lecacheux \n")
vieta_sum=calc_vieta_sum(j)
repi = [expr.subs({b: -(T**3 - 2*T**2 + 3*T - 3), c: -T**2, d: -1}).simplify().factor().subs({x0: 1}) for expr in vieta_sum]
...
print("\n Replaced by Kishi \n")
vieta_sum=calc_vieta_sum(j)
repi = [expr.subs({b: -(T*(T**2+T+3)*(T**2+2)), c: -(T**3+2*T**2+3*T+3), d: -1}).simplify().factor().subs({x0: 1}) for expr in vieta_sum]
disco=(repi[1]**2-4*repi[0]*repi[1]).full_simplify().factor()
for ico, termo in enumerate(repi):
    print("\n", ico, " term/coefficient:")
    print(termo, "\n")
